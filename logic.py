...
class LogicStore:
	...
	def __init__ (self, max_card_value=50, randomize_max_card_value=False, distribution=DEFAULT_DISTRIBUTION, num_players=1, include_sink=True, discard_unplayable=False):
		# Parameter-derived attributes
		self._max_card_value = max_card_value
		self._distribution = distribution
		self._num_players = num_players
		self._include_sink = include_sink
		self._discard_unplayable = discard_unplayable
	...
	def _remove_and_total_factors (self, choice):
		...
		for c in self._remaining_cards[:]:
			...
			if not factors_remaining and not multiples_remaining:
				if not self._discard_unplayable:
					total_points += c
				self._remove_card(c)
